  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      <div className="bg-panel border border-border rounded-xl p-8 text-center">
        ...
        <div className="font-mono text-xs text-[#6B7873] mb-1">
          TOTAL SCORE
        </div>
        <h2 className="font-display text-4xl text-[#F0F2F0] mb-1">
          {totalMarks}{" "}
          <span className="text-lg text-[#6B7873]">
            / {maxMarks} {marksLabel}
          </span>
        </h2>
      ...
      {topicBreakdown?.length > 0 && (
        <div className="bg-panel border border-border rounded-xl p-6">
          <div className="text-xs tracking-[2px] text-[#6B7873] mb-4">
            ACCURACY BY TOPIC
          </div>
          <div className="space-y-2">
            {topicBreakdown.map((t) => {
              ...
              return (
                ...
                >
                  <span className="text-[#E4E7E4]">{t.topic}</span>
...
function ScoreStat({ label, value, color }) {
  return (
    <div>
      ...
      <div className="font-mono text-[10px] tracking-wide text-[#6B7873] mt-1">
        {label.toUpperCase()}
      </div>
    </div>
  );
...
function TimeRow({ label, value, emphasize }) {
  return (
    <div className="flex justify-between">
      ...
      <span
        className={`font-mono ${emphasize ? "text-[#F0F2F0]" : "text-[#E4E7E4]"}`}
      >
        {value}
      </span>
    </div>
